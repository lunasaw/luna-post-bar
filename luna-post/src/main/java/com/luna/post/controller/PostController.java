...
public class PostController {
    ...
    public ResultDTO<PageInfo<Post>> listPageByEntity(@PathVariable(value = "page") int page, @PathVariable(value = "size") int size, Post post) {
        PageInfo<Post> pageInfo = postService.listPageByEntity(page, size, post);
        return new ResultDTO<>(true, ResultCode.SUCCESS, ResultCode.MSG_SUCCESS, pageInfo);
    }

    @GetMapping("/pageList/{page}/{size}")
    public ResultDTO<PageInfo<Post>> listPage(@PathVariable(value = "page") int page, @PathVariable(value = "size") int size) {
        PageInfo<Post> pageInfo = postService.listPage(page, size);
        return new ResultDTO<>(true, ResultCode.SUCCESS, ResultCode.MSG_SUCCESS, pageInfo);
    }

    @PostMapping("/insert")
    public ResultDTO<Post> insert(@RequestBody Post post) {
    ...
    public ResultDTO<Boolean> update(@RequestBody List<Post> list) {
        return new ResultDTO<>(true, ResultCode.SUCCESS, ResultCode.MSG_SUCCESS, postService.updateBatch(list) == list.size());
    }
